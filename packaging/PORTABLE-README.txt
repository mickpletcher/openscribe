...
The first desktop launch can connect OpenAI, Anthropic Claude, Google Gemini, Mistral, xAI Grok, DeepSeek, Azure OpenAI, FreeLLMAPI, LM Studio, another local model server, or another OpenAI-compatible provider. API keys are stored separately by provider in the operating system credential store, not in this portable folder.
