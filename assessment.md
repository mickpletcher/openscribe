...
**Last targeted safety review:** 2026-09-15
**Assessed implementation:** Current `main` worktree based on commit `97f1037`
**Basis:** Repository inspection; local Python 3.11 through 3.13 pytest runs; hosted Windows, Ubuntu, and macOS CI for the committed baseline; coverage; Ruff; Node task-pane tests; source, wheel, portable Windows, and Windows installer builds; installed-wheel, packaged-application, install, upgrade, and uninstall smoke tests; sample-index integrity; dependency audit; Bandit; secret scan; living-document compliance; three independent Claude review passes with local reproduction and correction of every confirmed defect; independent FreeLLMAPI trust-boundary review
**Assessed by:** Codex. Independent review of the 2026-09-15 FreeLLMAPI change identified one privacy disclosure gap, which was corrected and covered by desktop regression tests.
...
The desktop includes a draggable brainstorming flowchart that turns visible ideas into a previewed book outline. Directed links set reading order, note groups become manuscript parts, and note details become chapter synopses. Applying the outline is checkpointed, links created chapters back to their source ideas, and refuses cycles, missing targets, unresolved legacy links, and stale previews.

The desktop also offers optional first launch setup for OpenAI, Anthropic Claude, Google Gemini, Mistral, xAI Grok, DeepSeek, Azure OpenAI, Hugging Face, OpenRouter, FreeLLMAPI, LM Studio, Meta Llama through Ollama, OpenAI-compatible hosted APIs, and other local servers. Hugging Face and OpenRouter have named presets for their OpenAI-compatible hosted routers and require approval for every manuscript transfer. FreeLLMAPI has a named preset for its loopback OpenAI-compatible gateway and always requires hosted-transfer approval because the gateway forwards requests to external providers. LM Studio and Meta Llama through Ollama have named keyless presets for their standard loopback endpoints. **Write with AI** turns a writer description into a previewed approximate-page or complete-chapter draft. Accepted output remains an unsaved editor draft until the writer invokes the existing conflict-checked, checkpointed save path. Connection testing sends only a synthetic message and no manuscript text. Provider keys use separate operating system credential-store entries. Existing projects are not changed automatically. Hosted and non-loopback manuscript requests require approval for every transfer.
...
- Brainstorming outline creation reuses the existing board format, previews the result, validates directed links, rejects stale plans, and applies the complete manuscript and board update through the checkpointed staged transform. Evidence: `src/openscribe/board.py`, `src/openscribe/desktop.py`.
- CLI scene move, split, and merge apply paths create an automatic checkpoint before manuscript replacement and reject source changes detected before or after checkpoint creation. Snapshot restore creates an independent backup of the current project before applying the selected checkpoint, then uses staged replacement, a recovery journal, and rollback. Evidence: `src/openscribe/project.py`, `src/openscribe/editing.py`, `src/openscribe/snapshots.py`, `tests/test_cli.py`, `tests/test_project.py`.
...
- Hosted AI and LanguageTool requests require disclosure and explicit transfer approval based on the actual endpoint. Hugging Face and OpenRouter are always classified as hosted. FreeLLMAPI remains classified as hosted transfer even at its loopback gateway because it forwards content externally. Meta Llama through Ollama uses loopback by default; a non-loopback endpoint requires HTTPS and transfer approval. Evidence: `src/openscribe/ai.py`, `src/openscribe/network.py`, `src/openscribe/proofreading.py`, `docs/decisions/ADR-011-freellmapi-provider.md`.
...
- Tests: PASS on 2026-09-15. Final isolated local Python 3.11, 3.12, and 3.13 runs each reported 212 passed, 0 failed, and 0 skipped. The final coverage run reported 212 passed and 85.99 percent coverage.
- Word task pane: PASS on 2026-09-15. `node --test tests/word_addin.test.cjs` exited `0` with 8 passed, 0 failed, and 0 skipped. Rejected Word bridge requests passed 10 consecutive Windows response-delivery runs.
- Lint: PASS on 2026-09-15. `python -m ruff check src tests scripts/check-secrets.py scripts/check_markdown_links.py` exited `0` with no findings.
- Build and CLI smoke: PASS on 2026-09-15. `uv build --python 3.13 --no-create-gitignore` produced one source distribution and one wheel. The installed wheel and source checkout displayed the expected CLI, desktop, and Word help.
- Security and dependencies: PASS on 2026-09-15. The dependency audit found no known third-party vulnerabilities; the unpublished local package was skipped because it is not on PyPI. Bandit found no medium-or-higher issues. The secret scan found 0 unreviewed findings.
- Documentation: PASS on 2026-09-15. The living-document check reported no missing, invalid, or review rows. The link checker resolved 66 local links and heading anchors across 54 Markdown files, and all 6 executable beginner-workflow tests passed.
- Windows distribution: Version 0.1.2 was published as a public prerelease on 2026-09-15 from commit `97f1037`. Its hosted installer gate passed build, install, upgrade, shortcut, launch, uninstall, and project-preservation checks. The release provides a per-user installer, portable ZIP, and SHA-256 checksum file. Packages remain unsigned and retain the stable-release gate in VL-007. [Release v0.1.2](https://github.com/mickpletcher/openscribe/releases/tag/v0.1.2) and [run 35040232260](https://github.com/mickpletcher/openscribe/actions/runs/35040232260) contain the published artifacts and hosted evidence.
...
- Independent FreeLLMAPI review: The 2026-09-15 reviewer found that the consent gate was correct but the desktop did not disclose that the loopback gateway forwards manuscript text to hosted providers. The setup and per-request text now make that routing explicit, regression tests cover both disclosures, and re-review confirmed the finding was resolved with no remaining functional, privacy, security, or upstream-contract findings.
